#!/usr/bin/env python3
"""The FULL staged build — every extension enabled, the whole story at
300x, no hardware: auto-recognition, hands-off readiness, detections/NE107,
quarantine, the swap drill, agent approvals, evidence export.

    python3 demo_full.py       (~90 s, exits DEMO OK)
...
def main():
    ...
    with open(cfg_path, "w") as f:
        json.dump({
            ...
            "extensions": ["analyzer", "scheduler", "detections",
                           "authority", "exports"],
    ...
    try:
        ...
        # -- 4.5: agent asks, human decides ------------------------------------
        resp = post(base, "/v1/commands",
                    {"module": "vendor-x-01", "type": "take_control",
                     "actor": "agent:eddy-om@demo"})
        assert resp.get("approval_required"), "agent disruptive must defer"
        print(f"[4.5] agent requested take_control -> PENDING APPROVAL "
              f"(risk above agent authority)")
        post(base, f"/v1/approvals/{resp['approval_id']}",
             {"decision": "approve", "actor": "user:keaton"})
        wait_for(lambda: (lambda s_: s_ and s_["state"] == "done")(
            get(base, f"/v1/commands/{resp['command_id']}")),
            timeout=30, what="approved command execution")
        print("      human approved -> executed; both steps are audit evidence")

        # -- 4.6: export the record --------------------------------------------
        req = urllib.request.Request(
            base + "/v1/exports",
            data=json.dumps({"module": "nh4mod-b"}).encode(),
            headers={"Content-Type": "application/json"}, method="POST")
        with urllib.request.urlopen(req, timeout=30) as r:
            blob = r.read()
            count = r.headers.get("X-UII-Bundle-Count")
        print(f"[4.6] evidence bundle for the new module: {count} envelopes, "
              f"{len(blob)} bytes (manifest + jsonl + chain proof + README)")
